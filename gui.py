...
# ── 配色常量 (黑蓝高级调色板) ──
BG = "#121212"          # 深邃黑底色
BG_SURFACE = "#1E1E1E"  # 深灰表面
BG_INPUT = "#2C2C2E"    # 输入框深灰
FG = "#F5F5F7"          # 浅色主文本
FG_DIM = "#A1A1A6"      # 辅助文字银灰
ACCENT = "#3B82F6"      # 炫动蓝主色调
ACCENT_HOVER = "#60A5FA"# 悬停亮蓝
GREEN = "#30D158"       # 翡翠绿
RED = "#FF453A"         # 宝石红
YELLOW = "#FFD60A"      # 琥珀黄
GOLD = "#3B82F6"        # 炫动蓝
GOLD_HOVER = "#2563EB"  # 深蓝
BORDER = "#3A3A3C"      # 深灰边框
...
class OpenClawApp:
    ...
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("薛定猫openclaw一键配置工具")
        self.root.geometry("1120x850")
        self.root.resizable(True, True)
        self.root.minsize(950, 650)
        self.root.configure(bg=BG)
        self.root.attributes("-alpha", 0.0)  # 初始透明度设为 0，用于淡入特效
        self._center_window()
        self._setup_styles()
        ...
        # 绑定特效和事件
        self.root.after(100, self._fade_in)
    ...
    def _fade_in(self, alpha=0.0):
        ...
        if alpha < 1.0:
            ...
            self.root.after(20, self._fade_in, alpha)
        else:
            self.root.attributes("-alpha", 1.0)

    def _animate_title_color(self, step=0):
        """标题渐变发光呼吸特效"""
        colors = ["#3B82F6", "#60A5FA", "#93C5FD", "#2563EB", "#60A5FA", "#3B82F6"]
        if hasattr(self, "title_lbl") and self.title_lbl.winfo_exists():
            self.title_lbl.configure(foreground=colors[step % len(colors)])
            self.root.after(500, self._animate_title_color, step + 1)
    ...
    def _setup_styles(self):
        ...
        style.configure(".", background=BG, foreground=FG, fieldbackground=BG_INPUT,
                         ...
                         troughcolor=BG, selectbackground=ACCENT, selectforeground="#FFFFFF",
        ...
        style.configure("Url.TLabel", foreground=GOLD, background=BG,
        ...
        # Treeview
        style.configure("Treeview", background=BG_SURFACE, foreground=FG,
                 fieldbackground=BG_SURFACE, bordercolor=BORDER, font=("Microsoft YaHei UI", 9))
        style.configure("Treeview.Heading", background="#2C2C2E", foreground=ACCENT,
        ...
        # Scrollbar
        style.configure("TScrollbar", background="#2C2C2E", troughcolor=BG,
    ...
    def _build_ui(self):
        ...
        self.log_text = scrolledtext.ScrolledText(
            ...
            selectbackground=ACCENT, selectforeground="#121212",
        ...
        if os.path.exists(logo_path):
            try:
                ...
                # 根据原图宽度自动缩放至合适尺寸（约 80-100px 宽）
                width = self._logo_orig.width()
                if width > 100:
                    factor = width // 90
                    self._logo_img = self._logo_orig.subsample(factor, factor)
                else:
                    self._logo_img = self._logo_orig
                logo_lbl = ttk.Label(title_frame, image=self._logo_img, background=BG)
                logo_lbl.pack(side=tk.LEFT, padx=(10, 20))
        ...
        self.gateway_btn = ttk.Button(launch_row, text="🚀 启动 Gateway",
                                       command=self._toggle_gateway, style="Action.TButton", cursor="hand2")
        self.gateway_btn.grid(row=0, column=0, sticky=tk.EW, padx=(0, 5))

        ttk.Button(launch_row, text="📊 启动 Dashboard",
                   command=self._launch_dashboard, style="Action.TButton", cursor="hand2"
        ...
        for i, (key, label) in enumerate([
            ...
            toggle_btn = ttk.Button(key_grid, text="👁", width=3,
                                    ...
                                    style="Small.TButton", cursor="hand2")
        ...
        self.primary_combo = ttk.Combobox(primary_row, textvariable=self.primary_var,
                                           state="readonly", width=55, cursor="hand2")
        ...
        self.add_provider_combo = ttk.Combobox(add_frame, textvariable=self.add_provider_var,
                                                values=provider_labels, state="readonly", width=8, cursor="hand2")
    ...
    def _apply_hover_effects(self):
        """为所有 ttk.Button 自动增加悬停发光/浮动特效的回调"""
        for child in self._iter_children(self.root):
            if isinstance(child, ttk.Button):
                # 利用 ttk 的状态特征，再增加微小的 cursor 和反馈
                child.bind("<Enter>", lambda e, w=child: self._on_btn_hover(e, w))
                child.bind("<Leave>", lambda e, w=child: self._on_btn_leave(e, w))
    ...
    def _on_btn_hover(self, event, widget):
        widget.configure(cursor="hand2")

    def _on_btn_leave(self, event, widget):
    ...
    def _refresh_model_tree(self):
        ...
        self.model_tree.tag_configure("oddrow", background=BG_SURFACE)
        self.model_tree.tag_configure("evenrow", background=BG_INPUT)
        
        for i, m in enumerate(self.models):
            label = installer.PROVIDERS.get(m["provider"], {}).get("label", m["provider"])
            tag = "evenrow" if i % 2 == 0 else "oddrow"
            self.model_tree.insert("", tk.END, values=(label, m["id"], m["name"]), tags=(tag,))
